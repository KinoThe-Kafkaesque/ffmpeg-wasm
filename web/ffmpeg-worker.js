  ...
  if (ret === 0) {
    ...
    if (state.api.duration && hasExport("ffmpeg_wasm_duration_seconds")) {
      ...
      if (duration > 0) {
        ...
        state.durationUnknownLogged = false;
      } else if (!state.durationUnknownLogged && state.activeFile && isMp4Container(state.activeFile)) {
        state.durationUnknownLogged = true;
        postLog("MP4 duration unknown until moov atom is available (faststart recommended).");
      }
    }
    postStatus("Playing");
    emitStats(true);
...
const performSlowSeek = (target) => {
  ...
  stopStream()
    .then(() => {
      ...
      // Save seek state before destroy (destroyDecoder resets seeking/seekTarget)
      const savedSeeking = state.seeking;
      const savedSeekTarget = state.seekTarget;

      destroyDecoder();

      // Restore seek state after destroy
      ...
      state.waitingForData = false;
      state.draining = false;
      // For MP4 containers, keep all data so FFmpeg can seek to read moov atom
      if (state.activeFile && state.api.setKeepAll && hasExport("ffmpeg_wasm_set_keep_all")) {
        const needsFullBuffer = isMp4Container(state.activeFile);
        state.api.setKeepAll(state.ctx, needsFullBuffer ? 1 : 0);
      }
      // Set file size so FFmpeg can seek for moov atom in MP4
      if (state.activeFile && state.api.setFileSize && hasExport("ffmpeg_wasm_set_file_size")) {
        state.api.setFileSize(state.ctx, state.activeFile.size);
      }
      if (state.api.setAudioEnabled && hasExport("ffmpeg_wasm_set_audio_enabled")) {
...
const startSource = async ({ file, url, formatHint, bufferBytes }) => {
  ...
  if (state.api.setKeepAll && hasExport("ffmpeg_wasm_set_keep_all")) {
    // For MP4 containers, always keep all data so FFmpeg can seek to read moov atom
    const needsFullBuffer = file && isMp4Container(file);
    state.api.setKeepAll(state.ctx, seekable || needsFullBuffer ? 1 : 0);
  }
  if (state.api.setBufferLimit && hasExport("ffmpeg_wasm_set_buffer_limit")) {
    state.api.setBufferLimit(state.ctx, BUFFER_LIMIT_BYTES);
  }
